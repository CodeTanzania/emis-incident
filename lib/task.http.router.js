...
router.get(PATH_LIST, function getTasks(request, response, next) {
  ...
  Task.get(options, function onGetTasks(error, results) {
    // forward error
    if (error) {
      next(error);
    }
...
router.post(PATH_LIST, function postTask(request, response, next) {
  ...
  Task.post(body, function onPostTask(error, created) {
    // forward error
    if (error) {
      next(error);
    }
...
router.get(PATH_SINGLE, function getTask(request, response, next) {
  ...
  Task.getById(options, function onGetTask(error, found) {
    // forward error
    if (error) {
      next(error);
    }
...
router.patch(PATH_SINGLE, function patchTask(request, response, next) {
  ...
  Task.patch(id, patches, function onPatchTask(error, patched) {
    // forward error
    if (error) {
      next(error);
    }
...
router.put(PATH_SINGLE, function putTask(request, response, next) {
  ...
  Task.put(id, updates, function onPutTask(error, updated) {
    // forward error
    if (error) {
      next(error);
    }
...
router.delete(PATH_SINGLE, function deleteTask(request, response, next) {
  ...
  Task.patch(_id, patches, function onDeleteTask(error, deleted) {
    //forward error
    if (error) {
      next(error);
    }
