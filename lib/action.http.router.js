 ...
 * @apiSuccess {IncidentType} incidentType An incident type(or nature) under which
 * an action belongs.
 * @apiSuccess {Incident} incident An incident under which an action is applicable.
...
/**
 * @apiDefine Actions
 * @apiSuccess {Action[]} data List of actions
 * @apiSuccess {String} data._id Unique action identifier
 * @apiSuccess {IncidentType} data.incidentType An incident type(or nature) under
 * which an action belongs.
 * @apiSuccess {Incident} data.incident An incident under which a action is
 * applicable.
...
router.get(PATH_LIST, function getActions(request, response, next) {
  ...
  Action.get(options, function onGetActions(error, results) {
    // forward error
    if (error) {
      next(error);
    }
...
router.post(PATH_LIST, function postAction(request, response, next) {
  ...
  Action.post(body, function onPostAction(error, created) {
    // forward error
    if (error) {
      next(error);
    }
...
router.get(PATH_SINGLE, function getAction(request, response, next) {
  ...
  Action.getById(options, function onGetAction(error, found) {
    // forward error
    if (error) {
      next(error);
    }
...
router.patch(PATH_SINGLE, function patchAction(request, response, next) {
  ...
  Action.patch(id, patches, function onPatchAction(error, patched) {
    // forward error
    if (error) {
      next(error);
    }
...
router.put(PATH_SINGLE, function putAction(request, response, next) {
  ...
  Action.put(id, updates, function onPutAction(error, updated) {
    // forward error
    if (error) {
      next(error);
    }
...
router.delete(PATH_SINGLE, function deleteAction(request, response, next) {
  ...
  Action.patch(_id, patches, function onDeleteAction(error, deleted) {
    //forward error
    if (error) {
      next(error);
    }
